...
def bubble_sort(data, drawData, timeTick, listbox):
    ...
    for i in range(n):
        ...
        for j in range(0, n - i - 1):
            ...
            drawData(data, ['#4ee44e' if x == j else '#4ee44e' if x == j + 1 else '#3F497F' if x > n - i - 1 else '#ADD8E6' for x in range(len(data))])
            ...
            if data[j] > data[j + 1]:
                ...
                drawData(data, ['#ff8c8c' if x == j else '#ff8c8c' if x == j + 1 else '#3F497F' if x > n - i - 1 else '#ADD8E6' for x in range(len(data))])
                time.sleep(timeTick/2)
    # Update listbox after finishing
    listbox.delete(0, END)
    listbox.insert(END, "Algorithm Finished")
    # Update the canvas after completion
    drawData(data, ['#3F497F' for x in range(len(data))])
